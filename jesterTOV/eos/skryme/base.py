r"""Skryme equation of state for nuclear matter."""
...
class Skryme_EOS_model(Interpolate_EOS_model):
    r"""
    ...
    **Reference:** Skyrme (1959), Vautherin & Brink (1972)
    ...
    def __init__(
        ...
        self.ndat = ndat
        self.nmin_Skryme = self.nmin_Skryme_nsat * self.nsat
        self.n_Skryme = jnp.linspace(
            self.nmin_Skryme, self.nmax, self.ndat, endpoint=False
        )
        self.ns_spline = jnp.append(self.ns_crust, self.n_Skryme)
        self.n_connection = jnp.linspace(
            self.max_n_crust + 1e-5, self.nmin_Skryme, self.ndat_spline, endpoint=False
        )
    ...
    def eDenSky(self, ron: Array, rop: Array) -> Array:
        ...
        # Clip to avoid NaNs in powers for negative densities during root finding
        # ron = jnp.clip(ron, 1e-25, None)
        # rop = jnp.clip(rop, 1e-25, None)
        # ro = jnp.clip(ro, 1e-25, None)
    ...
    def compute_proton_fraction_exact(
        ...
        def betaHMnpe_optimistix(guessYe, nb):
            ...
            sol = optx.root_find(fn, optx.Newton(rtol=1e-8, atol=1e-8), z0, throw=False, max_steps=1000)
            return jax.lax.cond(
                sol.result == optx.RESULTS.successful,
                lambda _: sol.value,
                lambda _: optx.root_find(fn, optx.Dogleg(rtol=1e-8, atol=1e-8), z0, throw=False, max_steps=1000).value,
                operand=None
            )
    ...
    def construct_eos(
        ...
        if self.with_muon:
            ...
            proton_fraction = self.proton_fraction(None, self.n_Skryme)
            e_fraction = None
            muon_fraction = None

        # Calculate energy density for each density point
        n_n = self.n_Skryme * (1 - proton_fraction)
        n_p = self.n_Skryme * proton_fraction

        e_Skryme = self.eDenSky(n_n, n_p)
        ...
        p_Skryme = jax.vmap(pressure_from_eos)(self.n_Skryme, proton_fraction)
        ...
        if self.with_muon and e_fraction is not None:
            ...
            # Electron pressure
            K_Fe = (3.0 * jnp.pi**2 * self.n_Skryme * e_fraction_safe) ** (1.0/3.0) * utils.hbarc
            ...
            # Muon pressure
            K_Fmu = (3.0 * jnp.pi**2 * self.n_Skryme * muon_fraction_arr) ** (1.0/3.0) * utils.hbarc
            ...
            p_lepton = jnp.zeros_like(self.n_Skryme)
            e_lepton = jnp.zeros_like(self.n_Skryme)
            e_final_arr = None
            muon_final_arr = None

        p_total = p_Skryme + p_lepton

        # Add rest mass energy density (same as draft_skryme)
        ...
        if self.with_muon and e_final_arr is not None:
            n_mu = self.n_Skryme * muon_final_arr
            n_e = self.n_Skryme * e_final_arr
            rest_mass_energy_density += n_mu * utils.m_mu + n_e * utils.m_e

        e_total = e_Skryme + e_lepton + rest_mass_energy_density

        # Compute cs2 including lepton contributions
        cs2_Skryme = self.compute_cs2(
            self.n_Skryme, p_total, e_total, proton_fraction, e_final_arr
        )

        # Spline for speed of sound for the connection region
        cs2_spline = jnp.append(jnp.array(self.cs2_crust), cs2_Skryme)
        ...
        n = jnp.concatenate([self.ns_crust, self.n_connection, self.n_Skryme])
        cs2 = jnp.concatenate(
            [jnp.array(self.cs2_crust), cs2_connection, cs2_Skryme]
        )
...
def create_skryme_for_extension(
    nsat: Float,
    ...
    r"""
    Create a Skryme_EOS_model instance for use with CSE/peakCSE extensions.

    Args:
        nsat: Nuclear saturation density [:math:`\mathrm{fm}^{-3}`]
        nmin_Skryme_nsat: Starting density for Skyrme region as fraction of nsat
        nbreak: Break density for transition to extension region [:math:`\mathrm{fm}^{-3}`]
        ndat: Number of density points for discretization
        skryme_kwargs: Additional keyword arguments for Skryme_EOS_model
        proton_fraction_setting: Either "exact" (with muons) or "approx" (without muons)

    Returns:
        Skryme_EOS_model instance configured for the extension
    """
