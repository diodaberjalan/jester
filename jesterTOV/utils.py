...
def limit_by_MTOV_and_interpolate(
    ...
    if extra is not None:
        extra = jax.tree_util.tree_map(lambda val: val[sort_idx], extra)
    ...
    # Optimized boundary padding natively supported by XLA
    mask_padded = jnp.pad(is_stable, (1, 1), constant_values=False)
    diff = mask_padded[1:].astype(int) - mask_padded[:-1].astype(int)
    ...
    rs_new = cubic_spline(log_pc_query, log_pc, r)
    lambdas_new = cubic_spline(log_pc_query, log_pc, l)
    ms_new = cubic_spline(log_pc_query, log_pc, m)

    if extra is not None:
        extra_new = jax.tree_util.tree_map(
            lambda val: cubic_spline(log_pc_query, log_pc, val), extra
        )
...
def cubic_hermite_interp(
    ...
    # Execution block remains identical; XLA will automatically optimize searchsorted across mapped passes.
    indices = jnp.searchsorted(xp, x) - 1
    indices = jnp.clip(indices, 0, len(xp) - 2)
