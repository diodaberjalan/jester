...
#     # Multi-guess strategy: try several phi0 initial values and pick the first
#     # physically plausible result. 
#     phi0_candidates = jnp.array([phi0_guess])
...
#             phi0_init,              # p1
#             jnp.array(jnp.nan),     # p2
#             jnp.array(jnp.nan),     # p3
#             0.0,                    # m1
#             0.0                     # r1
#         )
...
#             i, phi0_val, R_prev, M_prev, phi_inf_prev, prev_x, prev_F, p1, p2, p3, m1, r1 = state
...
#             # Track historical data for NN payload
#             m1_new = jnp.where(i == 0, M, m1)
#             r1_new = jnp.where(i == 0, R, r1)
#             p2_new = jnp.where(i == 0, x_next_val[0], p2)
#             p3_new = jnp.where(i == 1, x_next_val[0], p3)

#             return (i + 1, x_next_val[0], R, M, F_curr[0], new_prev_x, new_prev_F, p1, p2_new, p3_new, m1_new, r1_new), None

#         def phase_loop(loop_state):
#             def cond(cond_state):
#                 i, _, _, _, phi_inf_val, _, _, _, _, _, _, _ = cond_state
...
#     # Run iteration for all candidate phi0 values and pick the first physical result
#     def eval_candidate(phi0_trial):
#         state = run_iteration(phi0_trial)
#         i, phi0_res, R_res, M_res, phi_inf_res, prev_x, prev_F, p1, p2, p3, m1, r1 = state
#         is_phys = (i < max_iterations) & (M_res / utils.solar_mass_in_meter < 20.0)
#         return (*state, is_phys)

#     n_candidates = len(phi0_candidates)
#     all_results = jax.lax.map(eval_candidate, phi0_candidates)

#     # Stack results: each is an array of shape (n_candidates,)
#     i_all = all_results[0]
#     phi0_all = all_results[1]
#     R_all = all_results[2]
#     M_all = all_results[3]
#     phi_inf_all = all_results[4]
#     prev_x_all = all_results[5]
#     prev_F_all = all_results[6]
#     p1_all = all_results[7]
#     p2_all = all_results[8]
#     p3_all = all_results[9]
#     m1_all = all_results[10]
#     r1_all = all_results[11]
#     phys_all = all_results[12]

#     # Pick the first physical candidate (relies on 0-based argmax returning the earliest)
#     first_idx = jnp.argmax(phys_all.astype(jnp.int32))
#     i_final = i_all[first_idx]
#     phi0_final = phi0_all[first_idx]
#     R_final = R_all[first_idx]
#     M_inf_final = M_all[first_idx]
#     phi_inf_final = phi_inf_all[first_idx]
#     prev_x_final = prev_x_all[first_idx]
#     p1_final = p1_all[first_idx]
#     p2_final = p2_all[first_idx]
#     p3_final = p3_all[first_idx]
#     m1_final = m1_all[first_idx]
#     r1_final = r1_all[first_idx]

#     returnNAN = jnp.all(~phys_all)
...
#         return (
#             M_inf_jordan, R_jordan, Lambda_T_J, Lambda_S_J, Lambda_ST1_J, Lambda_ST2_J, q,
#             i_final, p1_final, p2_final, p3_final, prev_x_final[0], phi0_final, m1_final, r1_final, pc
#         )
