...
def plot_eos(ns, ps, es, cs2, input_dict, nsat, save_path):
    ...
    plt.savefig(
        os.path.join(save_path, "eos_plot.pdf"), format="pdf", bbox_inches="tight"
    )
    plt.close()
...
def plot_mr_comparison(results, gr_radii, gr_masses, save_path):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_path, "mr_comparison.pdf"), format="pdf", bbox_inches="tight"
    )
    plt.close()
...
def plot_splits(rad_seg1, mass_seg1, rad_seg2, mass_seg2, save_path):
    ...
    plt.legend()
    plt.savefig(
        os.path.join(save_path, "split_segments.pdf"), format="pdf", bbox_inches="tight"
    )
    plt.close()


def plot_noise_addition(
    data_radii,
    data_masses,
    sample_radii,
    sample_masses,
    ...
    plt.ylabel("Masses")
    plt.savefig(
        os.path.join(save_path, "noise_injection.pdf"),
        format="pdf",
        bbox_inches="tight",
    )
    plt.close()
...
def plot_mock_flows_contours(
    flows, samples_m, samples_r, data_radii, data_masses, case_name, n_obj, save_path
):
    ...
    plt.scatter(
        data_radii, data_masses, alpha=0.3, label="Reference masses radii", color="gray"
    )
...
def plot_final_mock_likelihood(mock_ll, m_eos, r_eos, save_path):
    ...
    for k in range(mock_ll.K):
        ...
        plt.contour(
            ...
            M,
            prob,
            levels=[float(level_2sigma), float(level_1sigma)],
            colors=[color],
            alpha=0.8,
            linestyles=["dashed", "solid"],
        )
        ...
        plt.scatter(
            ...
            s=50,
            edgecolors="black",
            label="Data Centroids" if k == 0 else "",
        )
    ...
    plt.legend(
        custom_lines, [r"$1\sigma$", r"$2\sigma$", "Data Centroids", "Reference EOS"]
    )
    plt.title("Mock MR Likelihood Contours with Reference EOS")

    plt.savefig(
        os.path.join(save_path, "final_likelihood_contours.pdf"),
        format="pdf",
        bbox_inches="tight",
    )
    plt.close()
