...
def generate_prior_file(folder_name, sys=True, corr=True, skew=True):
    """
    Generates the prior.prior file in the specified folder.
    Currently uses static values but accepts sys, corr, and skew 
    flags for future dynamic parameterization.
    """
...
phi_c = UniformPrior(0.9999, 1, parameter_names=["phi_c"])
phi_inf_tgt = UniformPrior(0.9999e-3, 1e-3, parameter_names=["phi_inf_tgt"])
"""
#     prior_content = """E_sat = UniformPrior(-16.1, -15.9, parameter_names=["E_sat"])
# K_sat = UniformPrior(150.0, 300.0, parameter_names=["K_sat"])
# Q_sat = UniformPrior(-500.0, 1100.0, parameter_names=["Q_sat"])
