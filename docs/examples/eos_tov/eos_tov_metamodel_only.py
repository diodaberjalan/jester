...
# }
NEP_dict = {
    "E_sat": -15.451024510181709,
    "K_sat": 168.98175714704948,
    "Q_sat": -645.4341634743986,
    "Z_sat": -4854.55334332118,
    "E_sym": 33.11527652897152,
    "L_sym": 147.47942171365827,
    "K_sym": -207.75595490026768,
    "Q_sym": 31.162227083051675,
    "Z_sym": 4041.586944937484,
}
...
ns, ps, hs, es, dloge_dlogps, mu, cs2, extra = eos.construct_eos(
    NEP_dict, return_extra=True, calculate_durca=True
)
(
    ns_approx,
    ps_approx,
    ...
    cs2_approx,
    extra_approx,
) = eos_approx.construct_eos(NEP_dict, return_extra=True)
...
eos_data_approx = EOSData(
    ns=ns_approx,
    ps=ps_approx,
    hs=hs_approx,
    es=es_approx,
    dloge_dlogps=dloge_dlogps_approx,
    cs2=cs2_approx,
    mu=mu_approx,
    extra_constraints=extra_approx,
)
...
ax.scatter(
    n_durca / nsat, ye_durca, color="tab:orange", marker="x", label=r"$Y_{e}$ DUrca"
)
ax.scatter(
...
plt.plot(
    masses_a, jnp.power(10, log10pcs_a) / utils.MeV_fm_inv3_to_geometric, label="approx"
)
plt.plot(
    masses,
    jnp.power(10, log10pcs) / utils.MeV_fm_inv3_to_geometric,
    "--",
    label="exact",
)
...
nep_batch = {name: jnp.array(random_params[:, i]) for i, name in enumerate(param_names)}
...
for name, arr in zip(output_names, all_outputs):
    ...
    if isinstance(arr, dict):
        ...
        for sub_key, sub_arr in arr.items():
            ...
            if sub_arr.ndim > 1:
                row_is_nan = jnp.any(
                    jnp.isnan(sub_arr), axis=tuple(range(1, sub_arr.ndim))
                )
            else:
            ...
            if nan_count > 0:
                ...
                print(
                    f"  Indices: {indices[:10].tolist()}{'...' if len(indices)>10 else ''}"
                )
        continue
...
print("🔴 Red points = NaN (Failed)")
print("🔵 Blue points = Success")
...
for i in range(n_dims):
    for j in range(n_dims):
        ...
        # --- OFF-DIAGONAL PLOTS (Scatter) ---
            ...
            if np.sum(is_bad) > 0:
                ax.scatter(
                    x_data[is_bad], y_data[is_bad], c="red", s=20, alpha=0.9, marker="x"
                )
...
plt.suptitle("NaN Failure Analysis (Red = Failed)", y=0.92, fontsize=20)
plt.show()
...
n_cut = jnp.where(has_invalid, ns_all[jnp.arange(n_tests), first_invalid_idx], jnp.inf)
...
def get_active_mask(arr_shape, base_mask, arr_ns=None, n_cut_thresholds=None):
    ...
    if arr_shape == base_mask.shape:
        ...
        if arr_ns.ndim == 1:
            arr_ns = jnp.broadcast_to(
                arr_ns, (n_cut_thresholds.shape[0], arr_ns.shape[0])
            )
    ...
    for key in list(durca_all.keys()):
        if hasattr(durca_all[key], "shape"):
            active_mask = get_active_mask(
                durca_all[key].shape, cut_mask, durca_ns_arr, n_cut
            )
...
for name, arr in zip(output_names, all_outputs):
    if isinstance(arr, dict):
        ...
        for sub_key, sub_arr in arr.items():
            ...
            if sub_arr.ndim > 1:
                active_mask = get_active_mask(
                    sub_arr.shape, cut_mask, sub_ns_arr, n_cut
                )
...
plot_valid = (
    np.random.choice(valid_idxs, size=n_plot_valid, replace=False)
    if n_plot_valid > 0
    else []
)
plot_invalid = (
    np.random.choice(invalid_idxs, size=n_plot_invalid, replace=False)
    if n_plot_invalid > 0
    else []
)
...
ax_cs2.axhline(
    1.0, color="black", linestyle="--", linewidth=1.5, label=r"Causality ($c_s^2=1$)"
)
